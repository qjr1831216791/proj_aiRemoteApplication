...
| Sprint 9（已完成 v0.7.0） | 攻击面加固：工作台命令面硬化（双闸校验 + SHA256 去短路 + CSP）· Caddy 下载指纹锁定 · 443 入口 basic_auth 多账号门（单门模式）· 组件升级脚本；同迭代交付 install-client 退役（2026-09-13 验收 done，AC1~AC11 全勾，记录见 acceptance-manual） | [011-attack-surface-hardening](../specs/011-attack-surface-hardening/spec.md)、[012-install-client-retirement](../specs/012-install-client-retirement/spec.md) |
