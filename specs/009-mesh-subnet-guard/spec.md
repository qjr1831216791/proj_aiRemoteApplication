...
- [x] **AC1**: Given 本机存在活动物理网卡（IPv4 网段如 192.168.1.0/24），When 用户将组网虚拟网段改为与之重叠（同段/包含）的值并保存，Then 保存被拒绝，设置卡显示网段冲突的双语错误文案（含冲突网段信息），设置未写入磁盘（重载后仍为旧值）
- [x] **AC2**: Given 虚拟网段（如默认 10.126.126.0/24）与所有活动网卡网段均不重叠，When 保存，Then 保存成功（与现状行为一致，无误报）
- [x] **AC3**: Given 网卡信息不可得（枚举失败或无活动网卡），When 保存，Then 不阻断、保存成功（fail-open），且检测被跳过这一事实可经日志途径观察（不静默吞掉）
- [x] **AC4**: Given 装机向导组网分支填写的网段与物理网卡重叠，When 点击应用，Then 同一检测入口拦截并显示同类错误（与设置卡同口径，不允许绕过）

### US2: 作为维护者，我在发版前运行校验命令，希望自动发现版本号不一致或 CHANGELOG 漏收口，以避免 0.3.0 式漏 bump 再次发生。

- [x] **AC5**: Given `apps/workbench/package.json`、`src-tauri/Cargo.toml`、`src-tauri/tauri.conf.json` 三处版本一致，且 `CHANGELOG.md` 存在以该版本号命名的版本节，When 运行 `scripts/release-check.ps1`，Then 所有检查通过、退出码 0，输出各检查项结果汇总
- [x] **AC6**: Given 任一版本文件与其余不一致，When 运行，Then 退出码非 0，输出明确指出各文件的版本值与不一致点；全程只读，不修改任何文件
- [x] **AC7**: Given CHANGELOG 中不存在对应版本节（仅有 Unreleased），When 运行，Then 退出码非 0，提示先按发布步骤将 Unreleased 收口为版本节

### US3: 作为宿主机用户，我在真机执行随包的旧通道卸载脚本，清掉 frpc/ddns-go 残留，使磁盘暴露面与 v0.4.0 的代码状态一致。

- [x] **AC8**（真机手工验收）: Given 宿主机组网服务 EasyTierMesh 在线且栈 `.env` 已含腾讯云密钥，When 执行随包 `uninstall-legacy.ps1`，Then 台账全部 Done/Skipped（无 Failed、退出码 0）：`tasklist` 无 frpc/ddns-go 进程、ddns-go 自启任务已注销、栈目录退役文件（frpc.exe/ddns-go.exe/ddns-go.yaml/frpc-run.log）已删除、`.env` 的 `SAKURA_FRP_KEY` 行已移除且其余行保留；执行台账摘要回填 specs/008 的 acceptance-manual（AC10/AC11 备注区）

### US4: 作为成员设备（手机/PC）用户，我在工作台查看一份符合 EasyTier 官方 TOML 格式的成员入网配置，以便在移动端 App 里对照逐项输入，避免漏项错配。

- [x] **AC9**: Given 组网设置已保存（网络名与对端节点非空），When 打开组网卡的「成员入网配置」入口，Then 展示 TOML 配置文本：`network_identity.network_name` 为已保存实际值、`network_secret` 为占位符（真实密钥不出现在 APP 任何界面/日志）、每个 `[[peer]]` 为已配置对端节点，关键字段附注释说明对应移动端 App 的输入项
- [x] **AC10**: 配置文本支持一键复制，密钥占位旁有指引文案（填入运行 `set-mesh-secret.ps1` 时设定的密钥）
- [x] **AC11**: 生成的配置文本符合 EasyTier 官方 TOML 格式（自动化校验：Rust 侧单测将生成文本反序列化，断言字段与已保存组网设置一致）
...
- **推荐客户端指引**：折叠区附 Android 端**官方 EasyTier 客户端**下载指引（官方 Releases 链接 + APK 适配说明）；**不推荐第三方客户端**——第三方 Orbit 在移动端系统网络切换/重启后隧道易失效（控制面在线但数据面零流量，需重装或重启手机方能恢复），可靠性不满足入网指引的推荐标准（见变更记录 2026-09-11 T14）
...
| 2026-09-11 | AC1~AC11 全部验证通过，状态 in-progress → done | AC5~AC7/AC11 自动化实测、AC8 真机执行+机检核验（2026-09-11）、AC1~AC4/AC9/AC10 判定逻辑均有自动化覆盖、GUI/App 呈现按需求方签收指令确认（2026-09-11）；记录见 acceptance-manual |
| 2026-09-11 | 验收后补强（T13，不改 AC）：§4 新增「推荐客户端指引」边界——折叠区附推荐 Android 客户端（第三方 Orbit，原生渲染）与 Releases 下载链接；前端构建复跑绿、zh/en 键集对称 | 需求方鸿蒙真机实测：官方移动端为 Tauri+WebView 套壳、无 Google Play 的国产系统 WebView 陈旧致样式变形；原生渲染第三方客户端 Orbit 实测可用，需求方要求写入引导说明 |
| 2026-09-11 | 验收后补强（T14，不改 AC）：§4「推荐客户端指引」改为**仅推荐 EasyTier 官方客户端**（官方 Releases 链接 + APK 适配说明），撤销 Orbit 推荐；折叠区文案、链接与 i18n zh/en 同步 | 需求方鸿蒙真机实测：Orbit 在移动端停止/系统网络切换后隧道不重建（控制面在线但数据面零流量，仅重装或重启手机可恢复），可靠性不满足入网指引推荐标准 |
