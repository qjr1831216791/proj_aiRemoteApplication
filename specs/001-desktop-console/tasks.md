...
- [x] T14 设置页与语言体系：五项行为开关 + 语言设置（跟随系统/zh/en；切换**立即生效含托盘菜单重建**，并同步所有脚本 `-Lang`；`auto` 判定为纯函数——系统显示语言 zh→中文否则英文，单测覆盖）；端口/路径/域名只读卡（复制 + "修改须重跑安装脚本"指引）+ 打开日志目录（依赖: T5/T2/T7）（验收: AC21/22/25）<!-- 2026-09-09 完成：SettingsView 五行为开关——服务/程序自启先 set_autostart_services/set_autostart_app（返回 tookOver→toast 提示接管）成功后再 save_settings，联动/退出动作/启动开页即改即存；乐观更新+失败回滚+busy 键防在途重复点击。语言三段（auto/zh/en）经 set_language 即时生效：托盘菜单重建 + 脚本 -Lang 跟随（AC25，auto 纯函数单测在 lang::tests）。只读卡 3001/443/9876、D:\Software\cloudcli-https、ai.jackqi.cn（CopyButton 复制 + "修改须重跑安装脚本"指引；常量与 src-tauri/src/consts.rs 同步维护注释）；打开日志目录按钮（create_dir_all+open）。settings://repaired → toast（AC24；12:29 真机损坏设置留档回退已实证）。zh/en 词条各 ~80 键。受限：验证会话遭系统锁屏，语言切换与托盘重建未做可视化点击核验（语义由 set_language 单测 + 重建失败即 Err 的错误路径覆盖），留 T16 手工清单 AC21/22/25 -->
- [x] T15 低频操作区：安装/重装、升级 CloudCLI(-Update)、HTTPS 栈装机（install-https）、HTTPS 环境配置、客户端配置（可见交互窗）、ddns-go 管理页、打开工作台页面；UAC 拒绝/脚本失败的明确提示（依赖: T7）（验收: AC19/20）<!-- 2026-09-09 完成：ToolsSection 折叠区 7 项——安装/重装（-Update 升级、-UseMirror 镜像源两开关透传）、install-https、enable-https（UAC 提权可见窗 -NoExit）、install-client（普通可见交互窗）、ddns-go 管理页/打开工作台（open_external）。scripts_availability().available=false → 全部禁用并透传禁用原因（spec §4.5）；run_tool 失败（UAC 拒绝/脚本失败/派发异常）→ toast 双语文案不崩溃（AC20，错误码经 shell_error_text 本地化）。App.tsx 外壳：主界面/设置双视图切换、语言/设置/状态装配、toast 栈（6s 自消）、is_hidden_startup show 门控（T12/AC11）与 openPageOnStart 开页。真机渲染核验（锁屏前整屏截图，折叠区可见）；UAC 实弹交互留 T16 手工清单 AC19/20 -->
