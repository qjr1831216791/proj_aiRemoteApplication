...
- [ ] T1 获取 SakuraFrp 定制版 frpc（Windows amd64）入 `resources/bin/` + manifest 哈希登记 + `scripts/build.ps1`/`tools/sprint0/bin` 打包副本同步（依赖: 无）（验收: 打包含 frpc 且哈希校验通过）
...
- [ ] T9 「访问通道」卡：当前通道显示、切换确认流程（含风险/步骤说明、防重复点击、失败提示）、穿透配置引导态（依赖: T8）（验收: AC5/6/7/11）
- [ ] T10 隧道状态显示与 DNS 指引条：四态渲染、错误摘要、双向指引显隐（依赖: T8/T9）（验收: AC1/4/12/13）
- [ ] T11 双语文案全覆盖（中/英，沿 AC25 语言机制）（依赖: T9/T10）（验收: 002 同款语言切换检查）

## 阶段 4: 测试与收尾

- [ ] T12 自动化测试收口：`cargo test` 全绿（含既有 134 项回归）；判定函数覆盖率对齐 plan §6（依赖: T4-T8）
- [ ] T13 真机验收：外部独立网络探测 `https://ai.jackqi.cn/`、热点⇄WiFi 切换实测、杀 frpc 守护拉起实测、双通道切换回退实测；对照 spec 逐条勾选 AC（依赖: T12 + T3 需求方建隧道完成）
- [ ] T14 文档收口：`CHANGELOG.md` Unreleased 登记、`specs/MOC.md` 状态流转、手工验收清单回填（沿 001 acceptance-manual 先例）（依赖: T13）
