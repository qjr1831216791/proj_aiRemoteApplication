...
- [x] T16 栈目录权限加固（安全修复）：`install-https.ps1` 建目录后调用 `Protect-StackDir`——断开 `D:\` 默认继承（原继承把 `Authenticated Users` 授为可修改），只授「运行账户 ∪ 交互登录用户 + SYSTEM + Administrators」；取 SID 而非账户名规避中文编码坑，失败如实报红不静默放行（依赖: T3）（验收: 交付物安全属性——栈目录含 `.env` 与证书私钥，不得对本机其他账户可读写）。管理员端到端已由生产机装机实证覆盖（插件版 Caddy 上线 + 正式证书验通）

## 阶段 5: 测试与收尾

- [x] T12 自动化收口：`cargo test` 164 项全绿 + `npm run build` 通过（依赖: T3~T11）
- [x] T13 手工验收：GUI 交互项 + 真机装机全流程 + 旧机重跑无漂移——需求方 2026-09-10 确认真机验收通过（生产机实装插件版 Caddy），AC 已随验收勾选；未另立 acceptance-manual.md（依赖: T12）
- [x] T14 文档收尾：CHANGELOG Unreleased 登记、MOC 状态流转、`docs/research/sprint0-cloudcli-lan-deploy.md` §9.3「新机请用装机向导」指引（已存在，随验收复核）（依赖: T13）
