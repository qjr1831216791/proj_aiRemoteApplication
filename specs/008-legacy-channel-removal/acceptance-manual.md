...
>
> **状态：已验收**（2026-09-11 需求方签收——AC1 静态部分与 AC2 为 T20 实测/自动化覆盖（附录 A），其余验收项按需求方签收指令确认通过；AC10/AC11 的真机卸载已于同日补执行完成，残留核验记录见 §3 后注与 spec 009 T9）
...
| AC10 | ✅ | 需求方签收指令 2026-09-11；**同日真机补执行**（spec 009 T9）：四类残留全清（进程/自启任务/退役文件/密钥赋值行），组网与 3001/443 未受影响 |
| AC11 | ✅ | 需求方签收指令 2026-09-11——拒删逻辑同上；真机执行时腾讯密钥在场（Sakura 赋值行移除、TENCENT_* 两行完好） |
| AC12 | ✅ | T19/T20 核对（归档/MOC/CHANGELOG/.env.example/ADR-0004 五项落地）+ 签收指令 2026-09-11 |

- 验收人：JackQi（需求方）　日期：2026-09-11
- **签收依据**：需求方 2026-09-11 指令「所有验收项标记成功」——版本固定 v0.4.0 时一并签收。
- **真机补执行（2026-09-11，spec 009 T9）**：需求方运行随包 `uninstall-legacy.ps1` 后核验——`tasklist` 无 frpc/ddns-go；计划任务「ddns-go Sprint0 autostart」已注销；栈目录 frpc.exe / ddns-go.exe / ddns-go.yaml / frpc-run.log 均已清除；`.env` 中 `SAKURA_FRP_KEY` 赋值行已移除（头部遗留一行无值的说明性注释行，不生效、原样保留），`TENCENT_SECRET_ID/KEY` 两行完好；EasyTierMesh 服务 Running、3001/443 监听与 TUN 虚拟 IP 正常——现役链路零误伤。
