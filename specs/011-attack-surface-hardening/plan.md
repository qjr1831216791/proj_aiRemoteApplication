...
四个交付面共用一个原则：**在不动 spec 010 网络边界的前提下，给「边界后零认证」的事实补纵深**。P1 在 Rust 派发层（`scripts::tool_plan`）对 `domain` 与 `stack_dir` 双字段白名单校验，并在拼接 sink 对值参数统一单引号包裹（评审修正：stack_dir 与 domain 同走裸拼通道，只修其一链路仍活）；P2 走方案 A（URL 锁核心版本 + SHA256 下载校验，字节可复现性不过关当次切 B）；P3 用 caddy 原生 basic_auth 的**多账号形态**——账号在工作台设置区增删改（需求方 2026-09-13 拍板），事实来源为栈目录 `auth-accounts.json`（仅 bcrypt 哈希），Caddyfile 标记段由其再生（不走 .env/环境变量——三条 caddy 启动路径注入链不覆盖，空哈希会拒启），`caddy reload` 零停机生效；升级脚本独立于安装包（tools/），把「版本号/哈希/内置二进制/测试」四个手工动作收敛为一条命令。
...
| P3 账号管理 | 工作台设置区「访问账号」界面 + Rust `https_auth` 命令组；栈目录 `auth-accounts.json` 为事实来源 | 需求方拍板多账号且在工作台管理；JSON 事实来源 + 标记段再生避免手改 Caddyfile；账号粒度吊销 | 单账号固定 admin（评审建议，需求方改判）；独立 ps1 交互脚本（管理入口分裂，随变更取消） |
| P3 哈希生成 | Rust 命令 spawn 栈目录 `caddy.exe hash-password --algorithm bcrypt`（stdin 传入） | 用 caddy 自家实现保哈希格式兼容零风险；密码不进命令行；显式算法防上游默认漂移 | Rust bcrypt crate / PS 模块：新增依赖且哈希前缀（$2b$）兼容性需另验 |
...
### P3：basic_auth 多账号（工作台管理，需求方 2026-09-13 变更）

```
事实来源：栈目录 auth-accounts.json
  { "accounts": [ { "username": "jack", "hash": "$2a$14$..." }, ... ] }   # 仅哈希

Rust https_auth 模块（Tauri 命令组）：
  https_auth_list() -> Vec<账号摘要>          # 读 JSON（仅用户名等非敏感字段）
  https_auth_upsert(user, password)           # 用户名白名单/密码最小长度校验 →
  https_auth_remove(user)                     #   spawn caddy.exe hash-password
                                              #   --algorithm bcrypt（stdin 传明文）
  upsert/remove 共同收尾：
    写回 JSON → 由 JSON 再生 Caddyfile 标记段
...
标记段缺失（存量装机）→ 首次管理操作时定位 "${Domain}:443 {" site 块首插入。
移除账号：对新请求即时 401；既有 WS 长连接断开后无法重连（文案注明，
  必要时重启 caddy 彻底断开）。
密码明文仅在工作台内存瞬时经过（IPC → Rust String → 子进程 stdin），
  不落盘、不进 settings/日志/命令行（spec 变更记录已登记该放宽）。
顺手修（评审发现的既有 bug）：menu.ps1:94 直启 caddy 无任何 .env 注入，而
  install-https.ps1:21-22 文案声称「总控菜单会自动注入」——补注入或改道
  run-caddy-hidden.ps1，与文案对齐。
```

（set-https-password.ps1 随多账号变更取消——管理入口统一工作台。）
...
| 栈目录 `auth-accounts.json` | `{accounts:[{username,hash}]}`，仅 bcrypt 哈希无明文；用户名字符白名单 `[a-zA-Z0-9_-]{1,32}`，密码最小长度 8 | https_auth 命令组（事实来源） |
...
- 新增命令组 `https_auth_list` / `https_auth_upsert` / `https_auth_remove`（Rust `https_auth` 模块；密码经 Tauri IPC 进入内存后仅以 stdin 转发给 `caddy.exe hash-password`，不落盘不进命令行）；前端设置区「访问账号」管理界面（列表/新增/改密/移除）。
...
| AC11 | Rust 单测：用户名/密码校验、JSON 读写、标记段再生（含存量首插与多账号形态）、备份回滚纯逻辑；代码走查：密码不落盘/不进 settings/日志/命令行；真机：改密即时生效、构造非法段内容时 validate 失败自动回滚演示 |
...
| 密码经工作台内存（多账号管理的固有代价，需求方接受） | 前端被攻破时可植入后门账号（利用仍需先过组网边界） | 明文不落盘/不进 settings/日志/命令行；账号库在栈目录既有 ACL 内；与 run_tool 等既有本机能力同级——spec 变更记录已登记 |
| 移除账号后既有 WS 长连接残留至断开 | 被移除成员的已开终端短期续命 | 文案注明；必要时重启 caddy 彻底断开（AC8 口径） |

## 8. 影响范围

- **Rust**：`apps/workbench/src-tauri/src/scripts.rs`（validate_domain/validate_stack_dir + tool_plan Result 化 + visible_script_params 值包裹 + 单测）、`mesh.rs`（prepare_stack 去短路 + 单测）、`commands.rs`（错误分支透出，签名已兼容）、新增 `https_auth.rs` 模块（账号 CRUD + 标记段再生 + validate/reload 编排 + 单测）、`settings.rs`（normalize_stack_dir 校验）。
- **配置**：`tauri.conf.json`（csp）。
- **脚本**：`tools/sprint0/bin/install-https.ps1`（版本+哈希常量、下载校验先于落位、生成端空标记段、-CaddyZip 提示）、`tools/sprint0/bin/menu.ps1`（直启路径 .env 注入缺失的既有 bug 顺手修）。（set-https-password.ps1 随多账号变更取消；build.ps1 $ScriptSubset 无需改动。）
- **前端**：设置区新增「访问账号」管理界面（列表/新增/改密/移除，含确认与错误提示）。
