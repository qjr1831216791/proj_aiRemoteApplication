...
9. **AC9/AC10 分享链接导入**：候选池下方「从社区分享链接获取」点「获取节点」→ toast 报提取/新增数、候选 textarea 出现新节点（含 discussion 正文与评论里的节点）；手动删掉几条再保存，重启 APP 后保持
10. **AC11 拉取失败**：把链接改成 `https://github.com/EasyTier/EasyTier/discussions/999999`（不存在）→ 明确报错、候选池不动；改成 http:// 链接 → 提示仅支持 https
