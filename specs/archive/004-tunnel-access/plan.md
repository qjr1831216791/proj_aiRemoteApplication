...
| **本地网络拦截 frp 协议**（家宽/公司网/公共 WiFi 的 DPI、运营商封锁；2026-09-10 真机实测定案） | 隧道反复登录失败（`login to server failed`/EOF）、迟迟不"在线"，看板只能如实显示"连接中"，无自愈手段 | 登录失败提示透出"可尝试更换网络（如手机热点）"；诊断顺序：先看 frpc 日志是否 EOF，**先换网络验证再怀疑代码**；约束已记 spec §5 |
