...
- [010-lan-boundary-hardening](./010-lan-boundary-hardening/spec.md) — 局域网边界收口：443 防火墙改「源网段 + TUN 接口」双条件白名单、3001 直访默认退役 + 12h 自动回落例外开关、程序级规则旁路加固、443 归类告警退役（2026-09-13 验收 done——需求方复测拍板「唯一放行组合=同一WiFi+专用网络+例外放行」；含 AC11 程序级旁路加固，记录见其 acceptance-manual；发布 v0.6.0）
