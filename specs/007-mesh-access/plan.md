...
- **流量两段加密模型（如实）**：secure-mode 下宿主机 ↔ 持 Noise 能力的成员为 E2E 加密（中继只见密文）；**Android 官方 App v2.6.4 无 secure-mode 配置 UI**，以 legacy 客户端身份加入（取证初判服务端可接受旧客户端），该段走 network_secret 派生加密而非 Noise E2EE——对单用户私有网络可接受，AC9 的「强制 secure-mode」约束落点在**宿主机配置侧**（渲染器唯一 + 无关闭开关 + 启动前校验），T1 真机实证 Android 加入连通性。
...
```toml
instance_name = "ai-remote-workbench"
...
进程级参数不入 TOML，走服务 binPath：`easytier-core.exe -c <config> -r 127.0.0.1:15888 --file-log-dir <stack>/easytier/logs`（取证确认 rpc/日志参数仅在 CLI）。
...
| AC9 | 渲染模板断言 secure_mode.enabled=true 恒成立；apply 前校验对非 secure 配置返回拒绝（篡改模拟） | — |
...
| R4 | Android App 以 legacy 身份加入（该段非 Noise E2EE） | 手机段加密强度低于 secure-mode 标称 | 如实文档（两段加密模型）；单用户私有网络可接受；Windows 成员可完整 secure-mode；后续 EasyTier App 支持 后自然升级 |
