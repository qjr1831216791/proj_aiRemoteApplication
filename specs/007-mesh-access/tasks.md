...
- [ ] T16 组网诊断工具与成员指引修正（依赖: T9/T11；验收: AC13 + AC12 指引文案修正，spec/plan 变更记录 2026-09-11）：Rust `mesh_diagnostics` 命令（六项只读探测，plan §5.1 契约：服务态/密钥就绪/逐条 peers TCP 可达 3s 超时/成员清单含 no_addr 标记/本机虚拟网卡含虚拟 IP/域名解析对齐 + 虚拟 IP:443 可达；各项独立不阻断、结果不含密钥）+ 设置区组网卡「运行诊断」入口与六项清单 UI（`mesh.diag.*` 双语修复指引）+ 成员指引文案按 T14 实测修正（虚拟 IP 静态分配 .2 起步/主机名建议/判定标准 = 设备列表出现 ai-remote-workbench）+ acceptance-manual.md 回填 2026-09-11 联调实录（AC1/AC2 通过 + 三断点发现：企业 WiFi 拦 7910 出网、成员端 DHCP 不可用需静态 IP、退出收摊静默化 c1a4427）
