...
- [x] T16 组网诊断工具与成员指引修正（依赖: T9/T11；验收: AC13 + AC12 指引文案修正，spec/plan 变更记录 2026-09-11）：Rust `mesh_diagnostics` 命令（六项只读探测，plan §5.1 契约：服务态/密钥就绪/逐条 peers TCP 可达 3s 超时/成员清单含 no_addr 标记/本机虚拟网卡含虚拟 IP/域名解析对齐 + 虚拟 IP:443 可达；各项独立不阻断、结果不含密钥）+ 设置区组网卡「运行诊断」入口与六项清单 UI（`mesh.diag.*` 双语修复指引）+ 成员指引文案按 T14 实测修正（虚拟 IP 静态分配 .2 起步/主机名建议/判定标准 = 设备列表出现 ai-remote-workbench）+ acceptance-manual.md 回填 2026-09-11 联调实录（AC1/AC2 通过 + 三断点发现：企业 WiFi 拦 7910 出网、成员端 DHCP 不可用需静态 IP、退出收摊静默化 c1a4427）✓ 2026-09-11（实现：mesh.rs 诊断内核拆「纯函数 judge ×6 + IO 探测段」——probe_tcp 3s 超时/refused-timeout 二分、成员 no_addr 聚合、域名链路解析值与虚拟 IP:443 直连分测（DNS 未对齐与服务不通可区分）；命令层 spawn_blocking 不堵 UI（探测含网络 IO）；前端 DiagItem 类型 + api 封装 + 设置区组网卡「运行诊断」按钮与六行清单（notice--ok/warn 汇总 + ✗ 项修复指引 + detail 数据摘要小字）；i18n mesh.diag.* 17 键两侧同步 + meshPeerGuide 按实测改写；acceptance-manual 附录回填联调实录。测试 212 绿（+6：URI 解析/连接分类/序列化形态/六项判定矩阵/码序锁定/AC8 密钥穿底），npm build 绿。坑：raw string 断言 JSON 键值对的尾引号歧义（r#""ok":true""# 内容含尾引号）——改用转义字符串字面量消除歧义）
...
- [x] T15 文档同步收尾：CHANGELOG Unreleased 登记、`.env.example` SAKURA_FRP_KEY 标注停用后可清除、MOC 状态流转、spec.md AC 勾选与状态 done、README 通道描述核对；对照 DoD 清单收尾（先行部分 ✓ 2026-09-11：CHANGELOG Unreleased 登记 007 三 Added/一 Changed/一 Removed、`.env.example` SAKURA_FRP_KEY 停用可清除标注、MOC 007 行注记（顺带修正 secure-mode→legacy 陈旧表述）、README 核对无需改（根 README 仅 001 能力概述无通道表述，apps/workbench 无 README）；**待真机验收后收尾**：spec.md AC 勾选与状态 done、MOC 流转 done、DoD 清单勾选）✓ 2026-09-11 验收签收收尾（需求方指令「所有验收项标记成功」：spec.md 13 AC 全勾 + 状态 done、acceptance-manual §3 回填、MOC 流转、DoD 勾选——AC1/AC2 联调实测，余按签收指令确认）

## 完成标志（DoD 检查）

- [x] spec.md 中所有 AC 已逐条验证通过（2026-09-11 需求方签收：AC1/AC2 实测，余按签收指令确认）
- [x] 自动化测试全部通过（含新增：渲染/校验/编排/调和/状态判定/binPath 无密钥断言）
- [x] 相关文档已更新（plan/tasks/spec/MOC/CHANGELOG/调研报告事实修正）
- [x] 本文件全部任务勾选完毕
