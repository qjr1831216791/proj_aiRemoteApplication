...
- [ ] **AC12**: Given 006 装机向导 When 用户选择组网分支 Then 分支步骤为组网形态——「成员设备安装客户端指引（含官方下载地址）→ 输入网络名/密钥（密钥经脚本）→ 启动并校验组网在线 → A 记录指向虚拟 IP 的解析校验」；SakuraFrp 实名/建隧道/CNAME 操作步骤不再出现。（成员指引文案按 T14 实测修正——变更记录 2026-09-11：虚拟 IP 静态分配、主机名填写、入网判定标准）

### US5: 作为服务端用户，我希望有一键组网诊断，以便成员无法访问时自行定位断点，无需逐项手工排查。

- [ ] **AC13**: Given 组网通道任一状态 When 用户在设置区组网卡点击「运行诊断」Then 依次给出六项检查的通过/失败结论与可读修复指引：①服务运行状态（未装/停止/禁用/运行）②密钥就绪（缺失 → set-mesh-secret.ps1 指引）③对端节点 TCP 可达（逐条 peers 实测，失败提示当前网络可能拦截该端口——附换网建议）④组网成员清单（本机 + 各成员；**无虚拟 IP 的成员显式标记「未完成地址分配」**）⑤本机虚拟网卡就位（虚拟 IP 已配置在本机网卡）⑥域名全链路（公网解析值 = 虚拟 IP，且虚拟 IP:443 可达）；全程不呈现密钥内容（AC8 口径）。
