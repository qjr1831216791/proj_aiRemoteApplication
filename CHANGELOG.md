...
Sprint 4 进行中（Spec 005/006）：域名心跳检测与傻瓜式装机向导——核心实现落地、自动化测试全绿，真机/GUI 验收进行中。

### Added
- **傻瓜式装机向导（Spec 006，进行中）**：主窗口新增「装机向导」第三视图——五阶段（基础/腾讯云前置/HTTPS 栈/访问通道/收尾）检测驱动推进，已完成阶段自动跳过、失败单阶段重试，状态持久化支持中断续跑；新机三目标（局域网/域名直连/域名穿透）全程按提示输入参数完成，腾讯云与 SakuraFrp 等 web 端办理事项给分步文字引导、办完点「校验」自动核验（密钥有效性/域名存在/解析对齐调腾讯云 API 实测）；TLS 获取方式改为 Caddy tencentcloud 插件自治（签发+续期全自动，acme.sh 退役，ADR-0003）；新增 `set-tencent-key.ps1`（CAM 密钥不回显写 `.env`）与 `config-ddnsgo.ps1`（免管理页生成配置并拉起）；主看板装机未完成时显示引导条；CGNAT/蜂窝网络下直连如实警示并建议穿透
