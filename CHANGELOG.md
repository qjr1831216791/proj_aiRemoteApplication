...
Sprint 7 交付（Spec 009 调优迭代）：组网网段冲突阻断（spec 007 §6 决议落地）、成员入网配置展示（含推荐 Android 客户端指引）、发版校验脚本、真机旧通道清理收尾；另含通道体检端口占用文案修复与总控菜单访问地址分场景优化。009 于 2026-09-11 验收 done（AC5~AC8/AC11 实测、AC1~AC4/AC9/AC10 自动化覆盖 + 需求方签收，记录见其 acceptance-manual；验收后补强 T13 推荐客户端指引随版交付）。

交付: specs/009-mesh-subnet-guard

### Added

- **组网网段冲突阻断（Spec 009）**：保存/应用组网设置时检测虚拟网段与本机物理网卡网段重叠（双判据，EasyTier 自身 TUN 地址自动剔除），重叠即拒绝保存并在提示中列出冲突网段——spec 007 §6 决议落地；网卡信息不可得时放行保存并记日志（fail-open 不锁死）
- **成员入网配置展示（Spec 009）**：组网卡新增折叠区，一键生成并复制符合 EasyTier 官方 TOML 格式的成员入网对照清单（网络名实值 + 密钥**占位符** + 全部对端节点，字段注释标注对应 App 输入项）——手机端对照逐项输入免漏项错配；真实密钥仍不出现在 APP 任何界面（007 AC8 延续）；折叠区附 Android 端推荐客户端指引（第三方 Orbit，原生渲染，需求方鸿蒙真机实测，规避官方 App WebView 套壳的样式变形）与 Releases 下载链接
- **发版校验脚本（Spec 009）**：`scripts/release-check.ps1` 只读校验三处版本号一致（Cargo.toml 锚定 `[package]` 段）+ CHANGELOG 版本节存在，任一失败非零退出并逐项定位——0.3.0 式漏 bump 的机器化防呆
...
- **总控菜单访问地址分场景**：`menu.ps1` 选项 3 的域名地址不再对局域网用户标注「推荐」——局域网行注明「同 WiFi 直访」，域名独立成「跨网组网」行并注明「成员设备加入 EasyTier 组网后可达；非成员不可达是设计使然」（与 v0.4.0 双方案访问模型一致）
- **v0.4.0 后全仓瘦身**（工程清理，不改变用户可见行为）：Rust 死代码退役——`Script::StopServer` 整链（随包分发已无 `stop-server.ps1`，实际停止走程序内实现）、`ScriptOutcome::TimedOut`（全仓无构造点）、`PollerHandle`（装配即弃的停止句柄）、零引用常量与便捷方法，编译告警 14→2（剩余为 spec 007 网段冲突检测的未接线功能提示）；前端清理 i18n 死键 4 项（词典 250→246，zh/en 键集一致）、死 CSS 类 2 块、零引用的 api/类型导出；文档对齐——`tools/` 两级 README 断链修复与脚本清单补齐（run-caddy-hidden / set-tencent-key / mesh-service / set-mesh-secret）、访问模型表述对齐双方案实态、根 README 新增「远程访问方案」段、spec 005 模板残留死导航链删除；下版本调优候选沉淀为 [docs/research/post-v0.4-tuning-backlog.md](docs/research/post-v0.4-tuning-backlog.md)
...
- **组网访问通道（Spec 007）**：主界面「访问通道」卡新增第三通道「组网」（EasyTier 私有组网，设为推荐默认）——域名只解析到私有网段虚拟 IP，公网不可路由，**未持组网密钥的设备在网络层不可达**，宿主机零公网入站暴露；组网引擎以 Windows 服务承载（随包分发 + 哈希校验），崩溃系统自动拉回、开机先于登录可用、日常无需管理员弹窗；组网状态区实时呈现成员列表（名称/虚拟 IP/延迟/丢包）与四态（在线/连接中/离线/未配置）+ 稳定码失败摘要；组网设置卡（网络名/虚拟 IP/网段/对端节点列表行内编辑即时校验，公共节点无 SLA 支持多对端冗余）+ 服务安装/重启/停止入口；通道体检新增组网口径（组网服务/对端/DNS 对齐=A=虚拟 IP）
- **停用直连/穿透入口（Spec 007）**：设置页「通道停用」卡——停用后组件不再被守护/自启/开机启动链拉起，DNS 记录清理（穿透 CNAME 删除、A 记录指向虚拟 IP或按选择删除），看板显示「已停用」chip；重新启用须确认框附安全警示（公网暴露面回归）；停用穿透附 `SAKURA_FRP_KEY` 清除脚本入口（`clear-frp-key.ps1`，从栈 `.env` 移除该行、其余保留）
- **组网密钥脚本（Spec 007）**：`set-mesh-secret.ps1` 交互式写入（不回显输入两次、复核仅显末 4 位、直写栈目录文件）——密钥不经过 APP 界面/IPC/日志，组网服务命令行不含密钥（配置文件注入，修正 frpc 时代的命令行暴露面）；密钥未配置时拒绝启动组网并给脚本指引
- **傻瓜式装机向导（Spec 006）**：主窗口新增「装机向导」第三视图——五阶段（基础/腾讯云前置/HTTPS 栈/访问通道/收尾）检测驱动推进，已完成阶段自动跳过、失败单阶段重试，状态持久化支持中断续跑；新机三目标（局域网/域名直连/域名穿透）全程按提示输入参数完成，腾讯云与 SakuraFrp 等 web 端办理事项给分步文字引导、办完点「校验」自动核验（密钥有效性/域名存在/解析对齐调腾讯云 API 实测）；TLS 获取方式改为 Caddy tencentcloud 插件自治（签发+续期全自动，acme.sh 退役，ADR-0003）；新增 `set-tencent-key.ps1`（CAM 密钥不回显写 `.env`）与 `config-ddnsgo.ps1`（免管理页生成配置并拉起）；主看板装机未完成时显示引导条；CGNAT/蜂窝网络下直连如实警示并建议穿透
- **域名心跳检测（Spec 005）**：每 60 秒探测访问域名（8s 超时），连续 2 次失败地址区显示红色标记、恢复自动消除；失败分类（DNS/连接/TLS/超时/HTTP 码）悬停可见并如实标注"本机视角"；设置页可开关

### Changed
- **访问通道收敛为组网单通道（Spec 008）**：原三通道（直连/穿透/组网）单选卡重构为「组网通道（EasyTier）」卡——概览行常驻虚拟 IP（一键复制）与在线成员数；组网密钥写入/安装刷新服务/同步 DNS 从装机向导下沉为主看板常驻入口（装机后日常维护不再依赖向导）；通道体检六项口径更新（DNS 对齐=A=虚拟 IP、网络归类判"不适用"、组网客户端、本机组件、域名全链路）；DNS 指引常态轮询（30s），残留 CNAME 判旁路暴露面提示
...
- **直连通道（DDNS/ddns-go）彻底退役（Spec 008）**：`ComponentId::DdnsGo` 与启动/停止/探活/自启全链、ddns-go 管理页与密码重置入口（含 `config-ddnsgo.ps1`/`reset-ddns-password.ps1`）、DNS 直连/穿透 reconcile 与 yaml 凭据回退链（凭据单源栈 `.env`）全部移除；specs/003、004 随之归档
- 装机向导的 SakuraFrp 分支（Spec 007）：实名/建隧道/CNAME 操作步骤不再出现，由组网分支（默认推荐）取代——成员设备客户端指引 + 密钥脚本 + 在线校验 + DNS 同步
- i18n 词典收敛：双语各删约百条通道/停用/ddns 词条（穿透切换确认、通道标签、停用确认、ddns 工具等）
...
Sprint 2+3 交付（Spec 002/003/004）：网络环境反馈与防火墙归类调整、ddns-go 密码重置、内网穿透双通道与通道体检；另含 001 桌面控制台的试用反馈修复。

交付: specs/002-network-profile · specs/003-ddnsgo-password-reset · specs/004-tunnel-access
...
### Fixed
- 加固（栈目录权限）：`install-https.ps1` 建栈目录后新增 `Protect-StackDir`——断开 `D:\` 的默认权限继承，改为只授「运行账户 + SYSTEM + Administrators」。原继承把 `Authenticated Users` 授为可修改：本机任何登录账户既能读走 `.env` 里的 `SAKURA_FRP_KEY` / 腾讯云 SecretId・Key，也能覆写 `caddy.exe`（等于以托管账户执行任意代码）。既有部署重跑该脚本即收紧（真机已收紧并验证 HTTPS 栈正常）
- 穿透守护死锁：会话卡死自愈在持有状态锁时调用重启（内部二次加锁，Mutex 不可重入）致守护线程卡死、全部状态查询阻塞、UI 卡死（真机实证"反复点体检卡死"）——守护 tick 重构，IO 全部移出锁外
- 隧道"在线"误导：原判定只看进程存活，frpc 登录节点失败重试期间（实测可达 67 秒）仍显示在线——改用 frpc 日志实况判定，登录失败如实显示"连接中"并附失败详情
- 启动链通道互斥：穿透模式下自启/一键启动不再拉起 ddns-go（此前会被无脑拉起并把 A 记录重新写回 DNS，与 CNAME 混挂致解析不可靠——真机实证后回修）
- 隧道登录失败提示透出可执行建议：此前详情固定为"节点登录失败，自动重试中"，未透出 frpc 原话里的排查指引——改为稳定码 `login_failed` + 前端双语映射，明确提示"若长时间不恢复，可能是当前网络拦截了 frp，可尝试更换网络环境（如手机热点）"（真机定案：隧道迟迟不转绿的根因是**所在网络拦截 frp**，换热点即恢复）
- 隧道状态陈旧日志误判：守护判定登录实况读栈目录 `frpc-run.log`，而手动启动的 frpc 输出在控制台、不写该文件，上一会话残留亦会被读到，致"其实已通"被判成"连接中"——加新鲜度校验（仅本会话拉起过且日志写入不早于该次拉起才采信），异源 frpc 按"采纳 + 宽限期"判定
